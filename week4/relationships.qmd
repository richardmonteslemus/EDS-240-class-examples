...
Data are downloaded frominstall.packages("metajam") [DataOne](https://search.dataone.org/view/https%3A%2F%2Fpasta.lternet.edu%2Fpackage%2Fmetadata%2Feml%2Fknb-lter-hbr%2F208%2F14){target="_blank} (find instructions for getting the download link on the [`{metajam}` package README](https://github.com/NCEAS/metajam?tab=readme-ov-file#how-to-get-the-url-to-your-dataset-of-interest-){target="_blank}).
...
# Seems that when DOC increases pH decreases 
p1
```
...
# Adding histogram to plot to show distribution of data 
...
stream_chem_data %>% 
  ...
  ggplot(aes(x = DOC, y = pH)) + 
  geom_point(alpha = 0.5) + 
  geom_smooth(method = "lm", se = FALSE) # Use loess which is similar to moving average. 
# Got to remove se since it makes it seem like we calculated the error and that is not the case 
```
...
#   ggplot(aes(x = DOC, y = pH, color = site, size = Al_ICP)) + 
#   geom_point(alpha = 0.5) +
#   scale_radius(range = c(1, 10)) + # Radius is deceiving since a point 2 times larger actually scales bubbles by 4. 
#   labs(x = "DOC (mg/L)", size = "Al (mg/L)", color = "Site")
```
...
# Both allow you to see how a varible varies based on another variable 
```
...
# Overplotting due to many points so you cant even see trends. 
```
...
# Increase transparency and size 
ggplot(stream_chem_data, aes(x = SO4, y = pH)) + 
...
# Intercept is between a 1 by 1 inch square there are between 0 and 25 of of all data points 
...
# Within darkest gray we find about 50% of all data points, for the next level out, etc. 
